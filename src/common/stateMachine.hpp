...
// Author: yoxia
// Change history:
//      2021/03/01: This is the initialization version.
//
// Discription:
// Implematation common state machine
//
//
...
{
public:
	using Fsm = CstateMachine<TAGT, STATE_T, EVENT_T, logFlag>;
	class Ctransition
	{
	public:
		Ctransition(){
	...
	CstateMachine(TAGT * obj, STATE_T startState):
		_obj(obj),
	...
	void addState(STATE_T state, std::string name,
			      void(TAGT::*enter)(), void(TAGT::*exit)(),
			      EVENT_T(T::*dispatchEvent)(EVENT_ENUM) = NULL)
	{
		_stateMap[state] = CstateBase(state, name, enter, 
									  exit, dispatchEvent);
	}
